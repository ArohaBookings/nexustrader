...
def format_scaler(dashboard_data: Mapping[str, Any]) -> str:
    ...
    return _html_lines(
        ...
        [
            ...
            f"PF: <code>{_number(live.get('profit_factor'), 0.0):.2f}</code> | Payoff: <code>{_number(live.get('payoff_ratio'), 0.0):.2f}R</code> | Max DD: <code>{_number(live.get('max_drawdown_r'), 0.0):.2f}R</code>",
            f"Recent delta: <code>{_number(scaler.get('recent_delta_expectancy_r'), 0.0):.3f}R</code>",
...
def format_aggression(dashboard_data: Mapping[str, Any]) -> str:
    ...
    return _html_lines(
        ...
        [
            ...
            f"Autonomous base: <code>{bool(controller.get('autonomous_base_active'))}</code> | Owner unlock required: <code>{bool(controller.get('owner_unlock_required'))}</code>",
            ...
            f"Profit factor: <code>{_number(live.get('profit_factor'), 0.0):.2f}</code> | Payoff: <code>{_number(live.get('payoff_ratio'), 0.0):.2f}R</code> | Max DD: <code>{_number(live.get('max_drawdown_r'), 0.0):.2f}R</code>",
